...
import { useUpdateProductMutation } from '@/redux/ProductApi/ProductApi';
...
const page = ({params}:any) => {
    ...
    const [updateFunc,{isLoading:upLoading}] = useUpdateProductMutation()
    const [loading,setLoading] = useState(false)
    
    const {data,isLoading} = useGetCategoryQuery(1);
    console.log(data?.result?.result,'category')
        const {
        register,
        handleSubmit,
        watch,
